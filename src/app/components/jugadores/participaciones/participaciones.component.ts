...
export class ParticipacionesComponent {
  ...
  hideColumnsExcept(columnIndexesToShow: number[]): void {
    ...
    for (let i = 0; i < rows.length; i++) { // Filas
      for (let j = 0; j < rows[i].cells.length; j++) { // Columnas
        const val = Number.parseInt(rows[i].cells[j].textContent as string);
        if (!Number.isNaN(val)) console.log("Number");
        
        if (columnIndexesToShow[1] == j && !Number.isNaN(val) && val == 0) {
          rows[i].cells[j].style.display = 'none';
          rows[i].cells[columnIndexesToShow[0]].style.display = 'none';
        } else if (columnIndexesToShow.includes(j)) {
